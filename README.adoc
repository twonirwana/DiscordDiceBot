...
image:https://ko-fi.com/img/githubbutton_sm.svg[link="https://ko-fi.com/Y8Y21FG71V",height=20px]
...
image:image/RnRD6_red.png[image,600]

Valid for d6 and the supported color are: `red`.

The dice image created by ThChandee

==== HeroQuest

image:image/HeroQuestD6_white.png[image,600]

Valid for d6 and the supported color are: `white`.

The dice image created by Sibun@ and https://game-icons.net[game-icons.net]
...
All channel config commands have an optional `channel` option.
If the `channel` option is kept empty then the command will be applied to the current channel.
If a channel is specified in the `channel` option then the command will be applied in the specified channel and not to the current channel.
This is especially useful if the `channel` option is a forum because then all new posts in the forum get a copy of the configuration/alias.
...
|color on |`colorOn(<expression>, <in>, <color> ...)` |`colorOn(8d10, [9/10], 'red')` | each element in `<expression>` that is in the elements in `in` will be get the color `<color>`. `<color>` can be an empty list `[]` or literal `''` and thereby removing the current color. It is possible to add multiple <in>/<color> pairs to set different color for different values. If the <color> expression contains dice then they will be rolled once and used for all elements.
