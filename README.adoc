...
There are two options: `none` and `alies_black_gold`.
The `alies_black_gold` dice set was designed and contributed by https://www.instagram.com/alie_in_beanland/[alieinbeanland].
The images will only be shown if the following conditions are met:

* The `answer_format` is set to `full` or `without_expression`
* All dice musst be one of the following: d4, d6, d10, d8, d12, d20, d100
* No set of dice with more than 15 dice
* Not more the 10 sets of dice
* No multi line result

image:image/blackAndGoldExample.png[image,480]
...
Special thank for https://www.instagram.com/alie_in_beanland/[alieinbeanland ] design of the dice images.