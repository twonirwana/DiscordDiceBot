...
If a command creates a button with a label (usualy the text after the `@`) then it is possible to give the button an emoji.
It musst be a single emoji at the start of the label.
It can be a unicode emoji or a custom discord emoji (provided with emoji button, on the right side of the message input).
...
* Icons from https://game-icons.net/ under https://creativecommons.org/licenses/by/3.0/[CC BY 3.0]
