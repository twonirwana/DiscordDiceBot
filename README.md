...
| Boolean Label Result          | `<expression>[>,<,=,<=,>=,<>]<valueToCampare>?<resultIfTrue>:<resultIfFalse>` | `1d2=2?Head:Tail` | Compares the result of the dice expression with the given operator and value. If it is true then return the first text and if not the second as result. |
