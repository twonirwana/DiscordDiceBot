...
public class RpgSystemCommandPreset {
    ...
    public static Config createConfig(PresetId presetId, Locale userLocale) {
        return switch (presetId) {
            ...
            case DICE_CALCULATOR ->
                    new SumCustomSetConfig(null, string2ButtonIdLabelAndDiceExpression(I18n.getMessage("rpg.system.command.preset.DICE_CALCULATOR.expression", userLocale)),
            ...
            case CALL_OF_CTHULHU_7ED ->
                    new CustomDiceConfig(null, string2ButtonIdLabelAndDiceExpression(I18n.getMessage("rpg.system.command.preset.CALL_OF_CTHULHU_7ED.expression", userLocale)),
                            DiceParserSystem.DICE_EVALUATOR, AnswerFormatType.without_expression, null, new DiceStyleAndColor(DiceImageStyle.polyhedral_3d, DiceImageStyle.polyhedral_3d.getDefaultColor()), userLocale);
    ...
    public EmbedOrMessageDefinition createMessage(PresetId presetId, UUID newConfigUUID, long guildId, long channelId, Locale userLocale) {
        ...
        if (config instanceof CustomDiceConfig) {
            return startPreset((CustomDiceConfig) config, customDiceCommand, newConfigUUID, guildId, channelId);
        } else if (config instanceof SumCustomSetConfig) {
            return startPreset((SumCustomSetConfig) config, sumCustomSetCommand, newConfigUUID, guildId, channelId);
        } else if (config instanceof CustomParameterConfig) {
            return startPreset((CustomParameterConfig) config, customParameterCommand, newConfigUUID, guildId, channelId);
        }
