...
class SumDiceSetCommandTest {
    ...
    void getEditButtonMessage_limit() {
        Optional<String> res = underTest.getCurrentMessageContentChange(new SumDiceSetCommand.State("x2", ImmutableMap.of("d4", 51)), new SumDiceSetCommand.Config());
        assertThat(res).contains("100d4");
    }

    @Test
    void getButtonMessage() {
        String res = underTest.createNewButtonMessage(new SumDiceSetCommand.Config()).getContent();
        assertThat(res).isEqualTo("Click on the buttons to add dice to the set");
    }

    @Test
    void getButtonMessageWithState() {
        String res = underTest.createNewButtonMessageWithState(new SumDiceSetCommand.State("roll", ImmutableMap.of("d4", 51)), new SumDiceSetCommand.Config())
                .orElseThrow().getContent();
        assertThat(res).isEqualTo("Click on the buttons to add dice to the set");
    }
    ...
    void getEditButtonMessage(SumDiceSetCommand.State state, String expected) {
        Optional<String> res = underTest.getCurrentMessageContentChange(state, new SumDiceSetCommand.Config());
    ...
    void copyButtonMessageToTheEnd_rollNoConfig_false() {
        Optional<MessageDefinition> res = underTest.createNewButtonMessageWithState(new SumDiceSetCommand.State("roll", ImmutableMap.of()), new SumDiceSetCommand.Config());
        assertThat(res).isEmpty();
    }
    ...
    void rollDice_1d4plus1d6plus10() {
        EmbedDefinition res = underTest.getAnswer(new SumDiceSetCommand.State("roll", ImmutableMap.of(
                ...
                "d6", 1,
                "m", 10
        )), new SumDiceSetCommand.Config()).orElseThrow();
    ...
    void getButtonLayout() {
        List<ComponentRowDefinition> res = underTest.createNewButtonMessage(new SumDiceSetCommand.Config()).getComponentRowDefinitions();
